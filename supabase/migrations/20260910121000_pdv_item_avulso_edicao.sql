...
-- Edição de venda PDV: aceita item avulso sem produto_id e sem estoque.
...
BEGIN
  ...
  LOOP
    v_eh_avulso := public.vendas_item_payload_eh_avulso(v_item);
    ...
    IF v_eh_avulso THEN
      ...
      v_item_resolvido := public.vendas_resolver_item_avulso_interno(
        p_empresa_id,
        v_item
      );
      IF v_venda_item_id IS NOT NULL THEN
        v_item_resolvido := v_item_resolvido
          || jsonb_build_object(
            'desconto', coalesce(v_item_antigo.desconto, 0),
            'acrescimo', coalesce(v_item_antigo.acrescimo, 0),
            'valor_total', round(
              (v_qtd * (v_item_resolvido ->> 'valor_unitario')::numeric)
              - coalesce(v_item_antigo.desconto, 0)
              + coalesce(v_item_antigo.acrescimo, 0),
              2
            )
          );
      END IF;
    ELSE
      ...
      v_item_resolvido := jsonb_build_object(
        'origem_item', 'produto',
    ...
    v_desconto_itens :=
      v_desconto_itens + coalesce((v_item_resolvido ->> 'desconto')::numeric, 0);

    v_acrescimo_itens :=
      v_acrescimo_itens + coalesce((v_item_resolvido ->> 'acrescimo')::numeric, 0);

    v_resolvidos := v_resolvidos || jsonb_build_array(v_item_resolvido);
  END LOOP;
  ...
  FOR v_lock IN
    ...
    FROM (
      ...
      UNION
      SELECT NULLIF(value ->> 'produto_id', '')::uuid
      FROM jsonb_array_elements(v_resolvidos)
      WHERE NULLIF(value ->> 'produto_id', '') IS NOT NULL
    ) AS x
    WHERE x.produto_id IS NOT NULL
  ...
  FOR v_lock IN
    ...
    FROM (
      ...
      FROM jsonb_array_elements(v_resolvidos)
      WHERE coalesce(value ->> 'origem_item', 'produto') = 'produto'
        AND NULLIF(value ->> 'produto_id', '') IS NOT NULL
      GROUP BY 1
    ) AS nova
    LEFT JOIN (
      SELECT
  ...
  LOOP
    ...
    VALUES (
      p_empresa_id,
      p_venda_id,
      coalesce(NULLIF(v_item_resolvido ->> 'origem_item', ''), 'produto'),
      NULLIF(v_item_resolvido ->> 'produto_id', '')::uuid,
