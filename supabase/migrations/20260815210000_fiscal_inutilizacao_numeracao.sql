...
-- UltraPDV — inutilização real de numeração NF-e/NFC-e
-- 1) tipo de evento inutilizacao
-- 2) reserva ignora emissão já inutilizada e bloqueia a pendente
...
create unique index if not exists
  fiscal_emissao_eventos_inutilizacao_unica
...
begin
  ...
  -- Tentativa ativa: mesma chave, exceto números já mortos.
  select e.*
  into v_existente
...
'Reserva numeração com idempotência por chave. Ignora emissões inutilizadas para permitir nova tentativa da mesma venda. Bloqueia se houver aguardando_inutilizacao.';
