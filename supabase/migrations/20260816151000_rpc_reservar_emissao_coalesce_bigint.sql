...
-- Corrige pg_catalog.coalesce(bigint, integer) em rpc_reservar_emissao_fiscal.
-- search_path = '' impede o cast implícito integer → bigint.
-- Lógica de numeração inalterada: GREATEST(contador, max(numero)+1).
...
begin
  ...
  select pg_catalog.coalesce(pg_catalog.max(e.numero), 0::bigint) + 1::bigint
  into v_max_usado
  from public.fiscal_emissoes e
  where e.empresa_id = p_empresa_id
  ...
  v_numero := pg_catalog.greatest(
    pg_catalog.coalesce(v_numeracao.proximo_numero::bigint, 1::bigint),
    pg_catalog.coalesce(v_max_usado, 1::bigint)
  );
  ...
  set
    proximo_numero = v_numero + 1::bigint,
