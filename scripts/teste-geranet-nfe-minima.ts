 ...
 *
 * Objetivo: emitir UMA NF-e 55 mínima em HOMOLOGAÇÃO (ambiente 2)
 * diretamente em POST /api/v1/nfe/emitir, fora do fluxo UltraPDV.
 *
 * NÃO usa:
 * - fiscal_emissoes / fiscal_emissao_tentativas
 * - vendas / estoque / caixa / carteira / PIX
 * - RPC de claim / retry / numeração de produção
 *
 * Uso:
 *   npx tsx scripts/teste-geranet-nfe-minima.ts
 *   npx tsx scripts/teste-geranet-nfe-minima.ts --executar
 *
 * Tenant:
 *   DIAGNOSTICO_USUARIO_ID → usuarios_empresas.principal + ativo
 *   ou DIAGNOSTICO_EMPRESA_ID → somente essa empresa
 *   ou um único vínculo principal+ativo no banco local
 */
...
function sanitizarDiagnostico(valor: unknown): unknown {
  const sanitizado = sanitizarConsultaGeranet(valor);
  return mascararIdentificadores(sanitizado);
}
...
function omitirOpcionaisVazios(payload: Record<string, unknown>) {
  const nfe = payload.nfe;
  if (!nfe || typeof nfe !== "object" || Array.isArray(nfe)) {
    return payload;
  }

  const documento = nfe as Record<string, unknown>;
  const empresa =
    documento.empresa && typeof documento.empresa === "object"
      ? (documento.empresa as Record<string, unknown>)
      : null;
  ...
  for (const chave of [
    ...
    if (!texto(documento[chave])) {
      delete documento[chave];
    }
  }

  if (empresa) {
    if (!texto(empresa.informacaoComplementar)) {
      delete empresa.informacaoComplementar;
    }
    delete empresa.idCodigoSegurancaContribuinte;
    delete empresa.codigoSegurancaContribuinte;
    if (!texto(empresa.logomarca)) {
...
async function main() {
  ...
  const [{ data: empresa, error: empresaError }, { data: fiscal, error: fiscalError }, segredosResult] =
    await Promise.all([
      admin
        .from("empresas")
  ...
  if (uf !== UF_EMITENTE) {
    throw new Error(
      `UF da empresa ativa é ${uf || "(vazia)"}, não ${UF_EMITENTE}. Diagnóstico abortado para não misturar tenant/UF.`
    );
  }
  ...
  if (crt !== 1) {
    throw new Error(
      `CRT da empresa ativa é ${crt}, não 1. Diagnóstico abortado para não forçar regime de outra empresa.`
    );
  }
  ...
  const itemMontado = montarItemGeranet({
    ...
    produto: {
      codigo: "DIAG-GERANET-1",
      nome: "Item diagnostico homologacao",
      unidadeMedida: "UN",
      precoVenda: 1,
      tipoItem: "00",
    },
  ...
  const payload = omitirOpcionaisVazios(
    montarPayloadNfeGeranet({
      ...
      itens: [itemMontado.item],
    }) as unknown as Record<string, unknown>
  ) as ReturnType<typeof montarPayloadNfeGeranet> & {
    nfe: { valorTotal?: string };
  };

  payload.nfe.valorTotal = "1.00";

  const payloadSanitizado = sanitizarDiagnostico(payload);
  const estrutura = caminhosPropriedades(payloadSanitizado);

  const cabecalho = {
    ferramenta: "scripts/teste-geranet-nfe-minima.ts",
    producao: false,
  ...
  console.log("=== TESTE ISOLADO GERANET — NF-e 55 HOMOLOGAÇÃO ===");
  ...
  const resultado =
    http >= 500
      ? "INDETERMINADO"
      : http === 422
        ? "REPRODUZIDO_FORA_DO_FLUXO_ULTRAPDV"
