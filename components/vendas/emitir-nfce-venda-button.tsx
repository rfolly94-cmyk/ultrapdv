...
export function EmitirNfceVendaButton({
  vendaId,
  serie,
  ambiente = 2,
  rotulo = "Emitir NFC-e",
  ...
  async function emitir() {
    ...
    try {
      const response =
        await fetch(
          "/api/fiscal/geranet/nfce-emitir-venda",
          {
            method:
              "POST",
            headers: {
      ...
      setMensagem(
        data.autorizada
          ? `NFC-e autorizada. Série ${data.serie ?? "—"}, número ${data.numero ?? "—"}${data.cstat ? `, cStat ${data.cstat}` : ""}.`
          : data.mensagem ??
            "Emissão processada."
      );
      ...
      setMensagem(
        error instanceof Error
          ? error.message
          : "Falha inesperada ao emitir NFC-e."
      );
    } finally {
      setEnviando(false);
    }
